...
<div class="card" style="width: 50rem;">
    ...
    <embed type="application/pdf" src="../assets/file/{{ $data->file }}" width="600" height="400"></embed>
      ...
      <div class="container d-flex" style="justify-content:center; align-items:center;">
        <a href="/updatekeluar/{{ $data->id}}" class="btn btn-success " tooltip="" style="color: white;" data-original-title="" title="">Edit</a> 
        <form method="post">
          <a type="button" href="/deletekeluar/{{ $data->id}}" rel="tooltip" class="btn btn-danger ml-3" style="color: white;" data-original-title="" title="">
            Hapus
          </a>
        </form>
          
        <button type="submit" class=" btn btn-success bg-success ml-3"><a class="text-white " href="{{ asset('assets/file') }}/{{ $data-> file }}" download>Download</a></button>
      </div>
      <a href="/suratkeluar" class="bg-warning btn ml-3">Kembali</a>
</div>
    </center>
  </div>
</div>
